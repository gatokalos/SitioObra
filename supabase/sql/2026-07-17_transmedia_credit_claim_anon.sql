-- Claim anonymous GAToken ledger events into the authenticated account.
-- This makes GATokens earned on one device portable after the user signs in there.
...
begin
  ...
  one_time_events as (
    select distinct on (e.event_key) e.*
    from raw_identity_events e
    where
      e.event_key in (
        'sonoro_unlock',
        'graphic_unlock',
      ...
      or e.event_key like 'showcase_boost:%'
      or e.event_key like 'resonance:l3-reward:%'
    order by e.event_key, e.created_at asc, e.id asc
  ),
  identity_events as (
    select e.*
    from raw_identity_events e
...
create or replace function public.claim_transmedia_anon_credit_events(p_anon_id text)
returns jsonb
language plpgsql
security definer
set search_path = public
...
begin
  if v_user_id is null then
    raise exception 'auth user is required' using errcode = '28000';
  end if;

  if v_anon_id is null then
    v_state := public.get_transmedia_credit_state(null);
    return jsonb_build_object('ok', true, 'migrated_count', 0, 'state', v_state);
  end if;

  update public.transmedia_credit_events
  set
    user_id = v_user_id,
  ...
  return jsonb_build_object(
    'ok', true,
    'migrated_count', coalesce(v_migrated_count, 0),
    'state', v_state
  );
end;
$$;

revoke all on function public.claim_transmedia_anon_credit_events(text) from public;
grant execute on function public.claim_transmedia_anon_credit_events(text) to authenticated, service_role;

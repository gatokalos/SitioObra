...
const PortalLiteratura = () => {
  ...
  const [l1Done, setL1Done] = useState(() => readResonanceProgress('literatura').l1Done);
  const [l2Answer, setL2Answer] = useState(() => readResonanceProgress('literatura').l2Answer);
  const [experienceDone, setExperienceDone] = useState(() => readResonanceProgress('literatura').experienceDone);
  const [l2Done, setL2Done] = useState(() => readResonanceProgress('literatura').l2Done);
  const [l3Rec, setL3Rec] = useState(() => readResonanceProgress('literatura').l3Recommendation);
  const [bitacoraDone, setBitacoraDone] = useState(() => readResonanceProgress('literatura').bitacoraDone);
  const refreshL1 = useCallback(() => {
    const p = readResonanceProgress('literatura');

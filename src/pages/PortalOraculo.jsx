...
const PortalOraculo = () => {
  ...
  const [l1Done, setL1Done] = useState(() => readResonanceProgress('oraculo').l1Done);
  const [l2Answer, setL2Answer] = useState(() => readResonanceProgress('oraculo').l2Answer);
  ...
  useEffect(() => {
    ...
    try {
      const key = 'gatoencerrado:resonance:oraculo';
  ...
  const handleAnswerResonance = useCallback(() => {
    let videoSeen = false;
    try {
      const s = JSON.parse(localStorage.getItem('gatoencerrado:resonance:oraculo') || '{}');
